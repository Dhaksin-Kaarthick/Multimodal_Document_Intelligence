...
# --- SQLALCHEMY LOCAL DOCKER PERSISTENCE SYSTEM ---
...
# --- ENFORCED LOCAL DOCKER STORAGE PATHS ---
DATABASE_URL = "sqlite:////app/data/users.db"

# Auto-initialize the data directory inside the container if it doesn't exist
...
async def lifespan(app: FastAPI):
    global qdrant_db
    # Enforce standard storage inside the mapped docker data volume
    qdrant_db = QdrantClient(path="/app/data/documents.db")
...
# --- LOCAL ENVIRONMENT ALLOWANCE MATRIX ---
app.add_middleware(
    CORSMiddleware,
    # Open wide for seamless local dashboard file execution links
    allow_origins=["*"],
...
async def heavy_rag_ingestion_pipeline(file_path: str, file_extension: str, document_id: str, user_id: str, file_content: bytes = None):
    ...
    try:
        ...
        if file_extension in ['.jpg', '.jpeg', '.png']:
            # ✅ FIXED: Read the file bytes directly from the disk path safely inside the background thread
            if os.path.exists(file_path):
                with open(file_path, "rb") as img_file:
                    active_image_bytes = img_file.read()

                image_description = process_standalone_image(
                    active_image_bytes)
        ...
        if combined_context:
            ...
            print(
                f"⚠️ [WARNING] No context text extracted for Document: {document_id}")
...
async def upload_file(file: UploadFile = File(...), background_tasks: BackgroundTasks = BackgroundTasks(), user_id: str = Depends(get_current_user_id)):
    ...
    try:
        with open(file_path, "wb") as buffer:
            # Stream chunk allocations to protect RAM
...
async def ask_question(payload: QuestionRequest, user_id: str = Depends(get_current_user_id)):
    ...
    try:
      ...
      # Locate this exact block inside your ask_question function in Backend/app.py
        headers = {
            "Authorization": f"Bearer {OPENROUTER_API_KEY}",
            "Content-Type": "application/json"
        }
        ...
        request_body = {
            # ✅ CORRECT SLUG: OpenRouter's official smart auto-routing system
...
async def summarize_document(document_id: str, user_id: str = Depends(get_current_user_id)):
    if qdrant_db is None:
        raise HTTPException(
            status_code=503, detail="Database cluster link context is inactive.")
    try:
        # ✅ FIXED: Replaced undefined COLLECTION_NAME variable with the explicit collection string name
        result = qdrant_db.scroll(
        ...
        # Sort chunks sequentially to preserve original reading structure
        sorted_points = sorted(points, key=lambda x: int(x.payload.get(
            "chunk_id", 0)) if x.payload.get("chunk_id") is not None else 0)
        full_text_context = "\n".join(
        ...
        prompt = (
            ...
            "of the key facts in the text below. Write the executive summary bullet points now.\n\n"
            f"Text:\n{full_text_context}\n\n"
            "Summary:"
        )
        ...
        # Prints the explicit error statement directly to your terminal logs if it hits a roadblock
        print(f"🔥 [SUMMARIZE CRASH DETAILS]: {str(e)}")
        raise HTTPException(
